...
public class ViewDragHelper {
    ...
    public static final int EDGE_NULL = 1 << 4;
    ...
    public boolean isInterceptTouchEvent(MotionEvent ev, int status) {
        ...
        switch (ev.getAction()) {
            case MotionEvent.ACTION_MOVE:
                float lastX = ev.getX();
                float lastY = ev.getY();
                if (status == ViewDragHelper.EDGE_LEFT) {
                    back = lastX > this.lastX;
                } else if (status == ViewDragHelper.EDGE_RIGHT) {
                    back = this.lastX > lastX;
                } else if (status == ViewDragHelper.EDGE_TOP) {
                    back = lastY > this.lastY;
                } else if (status == ViewDragHelper.EDGE_BOTTOM) {
                    back = this.lastY > lastY;
                }
        }
        lastX = ev.getX();
        lastY = ev.getY();
        Log.i("gongpengming", "back=" + back);
        return back;
    }
    ...
    private boolean isScreenScllor;

    public void setScreenScllor(boolean screenScllor) {
        this.isScreenScllor = screenScllor;
    }
